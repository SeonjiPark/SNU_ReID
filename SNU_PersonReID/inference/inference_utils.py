...
def run_inference(model, val_loader, args, print_freq, use_cuda):
    embeddings = []
    paths = []
    ...
    for pos, x in enumerate(val_loader):
        # if pos % print_freq == 0:
        #     log.info(f"Number of processed images: {pos*int(args.test_batch_size)}")
...
def run_inference_list(model, query_list, args, use_cuda):
    embeddings = []
    model = model.cuda() if use_cuda else model

    for pos, x in enumerate(query_list):
        batch = [x, 0, ""]
        embedding, path = _inference(model, batch, use_cuda)
