...
def pairwise_distances(embeddings: torch.float32, metric: str = "euclidean") -> torch.float32:
    ...
    distances = torch.maximum(distances, torch.tensor(0.0).cuda())
    #distances = torch.maximum(distances.clone().detach(), torch.tensor(0.0)) ##########this is the line that makes the error
    #distances = torch.maximum(distances, torch.tensor(0.0)) ##########this is the line that makes the error
...
def triplet_loss(distances: torch.float, mask_pos: torch.bool, mask_neg: torch.bool, margin: float) -> torch.float:
    ...
    return torch.maximum(hardest_pos_dists - hardest_neg_dist + torch.tensor(margin).cuda(), torch.tensor(0.0).cuda())


class OctupletLoss(torch.nn.Module):
    ...
    def forward(self, embeddings: torch.float, labels: torch.float) -> torch.float:
        ...
        #pairwise_dist = torch.randn((32,32)).cuda()
        #print(labels)
        lbls_same = torch.eq(torch.unsqueeze(labels, 0), torch.unsqueeze(labels, 1))
        #print(lbls_same)
        not_eye_bool = torch.logical_not(torch.eye(batch_size, batch_size, device=lbls_same.device).bool()).cuda()
        #print(not_eye_bool)
        mask_pos = torch.eq(lbls_same, not_eye_bool).float().cuda()
        #print("pos",mask_pos)
        mask_neg = torch.logical_not(lbls_same).float().cuda()
       # print("neg",mask_neg)

        #x = torch.sum(embeddings,dim=1)
        #pairwise_dist = x.repeat(32,1)
        # TRIPLETS HR:HR ---------------------------------------------------------------
        dist_hrhr = pairwise_dist[0:batch_size, 0:batch_size]
        loss_hrhr = triplet_loss(dist_hrhr, mask_pos, mask_neg, self.margin)

        #loss_hrhr = torch.sum(embeddings, dim =1)[:16]
        ...
        #loss_hrlr = torch.sum(embeddings, dim =1)[:16]
        ...
        #loss_lrhr = torch.sum(embeddings, dim =1)[:16]
        ...
        #loss_lrlr = torch.sum(embeddings, dim =1)[:16]

        # Combination of triplet loss terms
        losses = torch.transpose(torch.tensor([self.configuration] * batch_size).cuda(), 0, 1) * torch.stack([
            loss_hrhr,
            loss_hrlr,
            loss_lrhr,
            loss_lrlr
        ])
        #summe = torch.sum(embeddings)
