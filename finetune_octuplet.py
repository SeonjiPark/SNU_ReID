...
class CTLModel_octuplet(ModelBase):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=None):
        # print("TRAINING STEP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        self.hparams.SOLVER.BASE_LR = 0.01
        if self.trainer.current_epoch >= 128:
            self.hparams.SOLVER.BASE_LR = self.hparams.SOLVER.BASE_LR /8
        elif self.trainer.current_epoch >= 126:
            self.hparams.SOLVER.BASE_LR = self.hparams.SOLVER.BASE_LR /4
        elif self.trainer.current_epoch >= 123:
            self.hparams.SOLVER.BASE_LR = self.hparams.SOLVER.BASE_LR /2
        ...
        if batch_idx == 0:
            print(self.hparams.SOLVER.BASE_LR)
        ...
        x_lr = torch.clone(x)
        x_lr = torch.nn.functional.interpolate(x_lr, scale_factor = 1/2, mode = 'bicubic')
        x_lr = torch.nn.functional.interpolate(x_lr, scale_factor = 2, mode = 'bicubic')


        _, features = self.backbone(x)
        _, features_lr = self.backbone(x_lr)

        features_cat = torch.cat((features, features_lr), dim = 0)

        octuplet_loss = self.octuplet_loss(features_cat, class_labels)
...
if __name__ == "__main__":
    ...
    print("RUN MAIN")
    print(cfg)
    run_main(cfg, CTLModel_octuplet, logger_save_dir)
