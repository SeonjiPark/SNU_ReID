...
class R1_mAP:
    def __init__(self, model, num_query, val_dataloader, max_rank=50, feat_norm=True):
        ...
        # try:
        #     self.save_root_dir = model.trainer.logger.log_dir
        # except:
        #     self.save_root_dir = model.trainer.logger[0].log_dir

        # try:
        #     self.dataset = self.val_dataloaders[0].dataset.samples
        # except:
        #     self.dataset = self.test_dataloaders[0].dataset.samples
        self.dataset = self.val_dataloader.dataset.samples
