...
from SNU_Person_Detection.execute import *
...
class ReIDPerson:
    ...
    def init_network(self):
        ...
        # Recognition Network
        # JH todo
        self.recognition_network = Recognition_Model(self.args, self.device)
        self.recognition_network.to(self.device)

        ################################################
        #    4. Load Detection / Recognition Network   #
        ################################################
        #JH todo
        recognition_checkpoint = torch.load(self.args.recognition_weight_file)
    ...
    def detect(self, dataset):
        ...
        detect_preds, GT_ids = do_detect(self.args, self.detection_network,)

        return detect_preds, GT_ids

    def reid(self, pred_querys, ids):
    ...
    def folder_file_to_tensor(self, imgname):
...
if __name__ == "__main__":
    ...
    print("Start Init")
    reidperson.initialize('detect.cfg')
    reidperson.init_device()
    
    print("Start Load Network")
    reidperson.network('detect.cfg')
    dataset = reidperson.loadDatas('dataset/images/01.png')
    
    print("Start Detection")
    detect_preds, GT_ids = reidperson.detect(dataset)
    print("Start Re-Identification")
    recog_result = reidperson.recognize(detect_preds, GT_ids)
